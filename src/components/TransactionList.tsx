...
const TransactionList: FC<Props> = ({
  ...
  return (
    <TouchableOpacity style={styles.list} onPress={() => onListPress(val)}>
      <View style={{ width: "2%", backgroundColor: val['status'] == "SUCCESS" ? '#50C878' : '#CC5500' }} />
      <View style={{ flex: 1, flexDirection: "row", paddingHorizontal: 20, alignItems: "center" }}>
        ...
        <View style={{ width: "32%", justifyContent: 'flex-end', alignItems: 'flex-end' }}>
          <View style={[styles.statusButton, { borderWidth: val['status'] == "SUCCESS" ? 0 : 2, borderColor: val['status'] == "SUCCESS" ? undefined : 'orange', backgroundColor: val['status'] == "SUCCESS" ? '#50C878' : 'white' }]}>
